...
class ProductRepository extends BaseRepository implements ProductContract
{
                ...
                foreach ($attributes['attribute'] as $key => $attribute) {
                    if(!is_null($attribute)){
                        foreach ($attribute['value'] as $vKey=>$value) {
                            if($value!= null){
                                $isProductCreated->productAttribute()->updateOrCreate(['value'=>$value,'attribute_id' => $key],[
                                    'attribute_id' => $key,
                                    'value' => $value,
    ...
    public function updateProduct($attributes, $id){
        ...
        if ($isProductUpdated) {
            ...
            $isProductRelatedSeoUpdated = $productData->seo()->updateOrCreate(['seoable_type'=>get_class($productData),'seoable_id'=>$productData->id],['body' => $attributes['seo']]);
            if (isset($attributes['attribute'])) {
