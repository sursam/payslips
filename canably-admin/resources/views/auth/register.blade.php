...
<section class="login-content">
    ...
    <div class="login-box">
        <form class="login-form register-form" action="{{ route('admin.register.post') }}" method="POST" role="form">
            @csrf
            <h3 class="login-head">SIGN UP</h3>
            <input type="hidden" name="invitation_id" value="{{ $id }}">
            <input type="hidden" name="invitation_token" value="{{ (string) $invitationToken }}">
            <div class="form-group o-form-wrapper">
                <label class="o-custom-label" for="email">E-Mail Address</label>
                <input type="email" class="o-form-element o-custom-input form-control disabled" value="{{ $email }}" disabled>
