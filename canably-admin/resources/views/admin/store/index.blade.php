...
@section('content')
    @include('admin.layouts.partials.page-title', [
        'html' => ['route' => route('admin.store.add'), 'text' => 'Add Store'],
    ])
    <!-- Table -->
    <div class="bg-white shadow-lg rounded-sm border border-slate-200 mb-8">
        <header class="px-5 py-4">
            <h2 class="font-semibold text-slate-800">All Store <span
                    class="text-slate-400 font-medium">({{ $stores->count() }})</span></h2>
        </header>
                ...
                <table class="table-auto w-full divide-y divide-slate-200 customdatatable">
                    ...
                    <thead class="text-xs uppercase text-slate-500 bg-slate-50 border-t border-slate-200">
                        <tr>
                            ...
                            <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="font-semibold text-left">Coordinates</div>
                            </th>
                            <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                    ...
                    <tbody class="text-sm" x-data="{ open: false }">
                        @forelse ($stores as $store)
                            ...
                            <tr>
                                ...
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div>{{ $store->full_address != '' ? substr(strip_tags($store->full_address), 0, 30) . '...' : '---' }}</div>
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div>{{ $store->zip_code }}</div>
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div>Latitude:-{{ $store->latitude }}</div>
                                    <div>Longitude:-{{ $store->longitude }}</div>
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    @switch($store->is_active)
                                        @case(1)
                                            <a href="javascript:void(0)" data-value="0" data-table="stores" data-message="inactive"
                                        ...
                                        @case(0)
                                            <a href="javascript:void(0)" data-value="1" data-uuid="{{ $store->uuid }}"
                                                data-table="stores" data-message="active"
                                    ...
                                    <div class="flex items-center text-center">
                                        <div class="m-1.5">
                                            ...
                                            <a class="btn btn-sm border-slate-200 hover:border-slate-300 text-slate-600"
                                                href="{{ route('admin.store.edit', $store->uuid) }}">
                                        ...
                                        <div class="m-1.5">
                                            ...
                                            <a class="btn btn-sm border-slate-200 hover:border-slate-300 text-rose-500 deleteData"
                                                data-table="stores" data-uuid="{{ $store->uuid }}"
        ...
        @include('admin.layouts.paginate', ['paginatedCollection' => $stores])
    @endsection
