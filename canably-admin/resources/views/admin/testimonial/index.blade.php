...
@section('content')
    @include('admin.layouts.partials.page-title', [
        'html' => ['route' => route('admin.testimonial.add'), 'text' => 'Add Testimonial'],
    ])
    <!-- Table -->
    <div class="bg-white shadow-lg rounded-sm border border-slate-200 mb-8">
        <header class="px-5 py-4">
            <h2 class="font-semibold text-slate-800">All Testimonial <span
                    class="text-slate-400 font-medium">({{ $listTestimonials->count() }})</span></h2>
        </header>
            ...
            <div class="overflow-x-auto">
                <table class="table-auto w-full divide-y divide-slate-200 customdatatable">
                    ...
                    <tbody class="text-sm" x-data="{ open: false }">
                        @forelse ($listTestimonials as $listTestimonial)
                            <!-- Row -->
                            <tr>
                                ...
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div class="flex items-center text-slate-800">
                                        <div
                                            class="w-10 h-10 shrink-0 flex items-center justify-center bg-slate-100 rounded-full mr-2 sm:mr-3">
                                            <img class="ml-1" src="{{ $listTestimonial->display_image }}" width="20"
                                                height="20" alt="Icon 01" />
                                        </div>
                                    </div>
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div class="font-medium text-slate-800">{{ $listTestimonial->name }}</div>
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div class="font-medium text-slate-800">{{ $listTestimonial->product }}</div>
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div>{{ $listTestimonial->description != '' ? substr(strip_tags($listTestimonial->description), 0, 50) . '...' : '---' }}</div>
                                </td>
                                <td
                                    ...
                                    <div class="flex items-center text-center">
                                        <div class="m-1.5">
                                            ...
                                            <a class="btn btn-sm border-slate-200 hover:border-slate-300 text-slate-600"
                                                href="{{ route('admin.testimonial.edit', $listTestimonial->uuid) }}">
                                        ...
                                        <div class="m-1.5">
                                            ...
                                            <a class="btn btn-sm border-slate-200 hover:border-slate-300 text-rose-500 deleteData"
                                                data-table="reviews" data-uuid="{{ $listTestimonial->uuid }}"
        ...
        @include('admin.layouts.paginate', ['paginatedCollection' => $listTestimonials])
    @endsection
