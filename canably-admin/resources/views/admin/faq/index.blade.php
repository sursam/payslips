...
@section('content')
    @include('admin.layouts.partials.page-title', [
        'html' => ['route' => route('admin.faq.add'), 'text' => 'Add Faq'],
    ])
    <!-- Table -->
    <div class="bg-white shadow-lg rounded-sm border border-slate-200 mb-8">
        <header class="px-5 py-4">
            <h2 class="font-semibold text-slate-800">All Faqs <span
                    class="text-slate-400 font-medium">({{ $listFaqs->count() }})</span></h2>
        </header>
                    ...
                    <tbody class="text-sm" x-data="{ open: false }">
                        @forelse ($listFaqs as $faq)
                            ...
                            <tr>
                                ...
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div>{{ $faq->question != '' ? substr(strip_tags($faq->question), 0, 25) . '...' : '---' }}</div>
                                </td>
                                 <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div>{{ $faq->answer != '' ? substr(strip_tags($faq->answer), 0, 50) . '...' : '---' }}</div>
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    @switch($faq->is_active)
                                        @case(1)
                                            <a href="javascript:void(0)" data-value="0" data-table="faqs" data-message="inactive"
                                                data-uuid="{{ $faq->uuid }}"
                                        ...
                                        @case(0)
                                            <a href="javascript:void(0)" data-value="1" data-uuid="{{ $faq->uuid }}"
                                                data-table="faqs" data-message="active"
                                    ...
                                    <div class="flex items-center text-center">
                                        <div class="m-1.5">
                                            ...
                                            <a class="btn btn-sm border-slate-200 hover:border-slate-300 text-slate-600"
                                                href="{{ route('admin.faq.edit', $faq->uuid) }}">
                                        ...
                                        <div class="m-1.5">
                                            ...
                                            <a class="btn btn-sm border-slate-200 hover:border-slate-300 text-rose-500 deleteData"
                                                data-table="faqs" data-uuid="{{ $faq->uuid }}"
        ...
        @include('admin.layouts.paginate', ['paginatedCollection' => $listFaqs])
    @endsection
