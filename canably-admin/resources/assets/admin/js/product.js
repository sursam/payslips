...
$(document).ready(function() {
    ...
    $('body').on("click",".featureImage",function(e){
        e.preventDefault();
        ...
        $.ajax({
            type: "post",
            url: baseUrl+'ajax/makeFeaturedImage',
            data:{'uuid': uuid, 'table': table,'product':product},
            dataType: "json",
            success: function (response) {
                if(response.status){
                    $('.productImages').html(response.data.html);
                    $('#productContainer').html(response.data.products);
                    showToast('success','Featured Image','Image Featured Successfully');
                }
            }
        });
    });
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "delete",
                    method: "post",
                    url: baseUrl+"ajax/remove-media",
                    data: {'uuid': uuid},
                    dataType: "json",
                    success: function (response) {
                        $this.parent().parent().addClass("d-none");
                        if(response.status){
                            showToast("success","image",response.message);
                        }else{
                            showToast("error","image","Something went wrong");
                        }
    ...
    $('.viewImages').click(function (e) {
        ...
        $.ajax({
            type: "get",
            url: baseUrl+'ajax/getProductImages/'+uuid,
            dataType: "json",
            success: function (response) {
                if(response.status){
                    $('.productImages').html(response.data.html);
