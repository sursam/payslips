    ...
    <div class="grid grid-cols-12 gap-6">
        ...
        <div class="col-span-12 lg:col-span-12 2xl:col-span-12">
            ...
            <form class="user formSubmit fileUpload" enctype="multipart/form-data" method="post"
                action="{{ url('admin/grant/module/field-edit', [$uuid]) }}" id="pagesubmit">
                @csrf
                <div class="intro-y box lg:mt-5">
                    <div class="p-5">
                        <div>
                            ...
                            <div class="input-group mb-3">
                                <select disabled name="type" id="type"
                                    ...
                                    <option value="text" {{ trim($ModuleField->type) == 'text' ? 'selected' : '' }}>
                                        Text
                                    </option>
                                    <option value="selectbox"
                                        {{ trim($ModuleField->type) == 'selectbox' ? 'selected' : '' }}>
                                        Select Box</option>
                                    <option value="textarea" {{ trim($ModuleField->type) == 'textarea' ? 'selected' : '' }}>
                                        Textarea</option>
                                    <option value="number" {{ trim($ModuleField->type) == 'number' ? 'selected' : '' }}>
                                        Number
                                    </option>
                                    <option value="radio" {{ trim($ModuleField->type) == 'radio' ? 'selected' : '' }}>
                                        Radio
                                    </option>
                                    <option value="checkbox" {{ trim($ModuleField->type) == 'checkbox' ? 'selected' : '' }}>
                                        Check
                                        Box</option>
                                    <option value="date" {{ trim($ModuleField->type) == 'date' ? 'selected' : '' }}>
                                        Date</option>
                                </select>
                            </div>
                            @error('type')
                        ...
                        <div>
                            ...
                            <div class="input-group mb-3">
                                <input type="text" name="question"
                                    ...
                                    aria-describedby="basic-addon2" value="{{ $ModuleField->question ?? '' }}">
                            </div>
                            @error('question')
                                <span class="invalid-feedback d-block text-danger" role="alert">
                                    <strong>{{ $message }}</strong>
                        ...
                        <div>
                            ...
                            <div class="input-group mb-3">
                                <select name="is_mandatory" id="is_mandatory"
                                    ...
                                    <option value="0" {{ trim($ModuleField->type) == 0 ? 'selected' : '' }}>No
                                    </option>
                                    <option value="1" {{ trim($ModuleField->type) == 1 ? 'selected' : '' }}>Yes
                                    </option>
                                </select>
                            </div>
                            @error('is_mandatory')
                        ...
                        <div>
                            ...
                            <div class="input-group mb-3">
                                <input type="text" name="place_holder"
                                    ...
                                    aria-describedby="basic-addon2" value="{{ $ModuleField->place_holder ?? '' }}">
                            </div>
                            @error('place_holder')
                                <span class="invalid-feedback d-block text-danger" role="alert">
                                    <strong>{{ $message }}</strong>
                        ...
                        <div>
                            ...
                            <div class="input-group mb-3">
                                <input type="text" name="max_length"
                                    ...
                                    aria-describedby="basic-addon2" value="{{ $ModuleField->max_length ?? '' }}">
                            </div>
                            @error('max_length')
                                <span class="invalid-feedback d-block text-danger" role="alert">
                                    <strong>{{ $message }}</strong>
                        ...
                        {{-- <div class="optionBtn"
                            {{ !empty($ModuleField->ModuleFieldOption->toArray()) ? 'style=display:block' : 'style=display:none' }}>
                            ...
                            <div class="input-group mb-3">
                                <div class="form-check">
                                    <input name="option-field" class="form-check-input checkForAddOption" type="checkbox" value=""
                                        id="flexCheckDefault"
                                        {{ $ModuleField->ModuleFieldOption != null ? 'checked' : '' }}>
                        ...
                        @if ($ModuleField->ModuleFieldOption)
                            <div class="option"
                                {{ !empty($ModuleField->ModuleFieldOption->toArray()) ? 'style=display:block' : 'style=display:none' }}>
                                @foreach ($ModuleField->ModuleFieldOption as $key => $optionValue)
                                    @if ($key == 0)
                                    ...
                                    @else
                                        <div class="appendDiv grid grid-cols-12 gap-6 p-2"><br><input required type="text"
                                                class="form-control col-span-8 lg:col-span-8 2xl:col-span-8"
                                                name="option[]" placeholder="Option Value"
