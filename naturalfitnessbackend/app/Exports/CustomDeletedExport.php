...
class CustomDeletedExport implements FromCollection, WithHeadings, WithMapping
{
    public $role;
    public $orderBy;
    public $sortBy;
    public function __construct($role, $orderBy = 'id', $sortBy = 'DESC')
    {
        $this->role = $role;
    ...
    public function collection()
    {
        ...
        $sortBy = $this->sortBy;
        $model = new User;
        $model = $model->where(['is_registered' => 1]);
        $model = $model->onlyTrashed()->whereHas('roles', function (Builder $query) use ($role) {
    ...
    public function map($row): array
    {
        ...
        $userEmailArr = explode('-', $row->email);
        ...
        return [
            ...
            $row->deleted_at->format('d/m/Y') .' at '.$row->deleted_at->format('h:i A'),
