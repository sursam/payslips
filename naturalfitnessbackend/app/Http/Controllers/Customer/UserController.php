...
class UserController extends BaseController
{
    ...
    {
        $request = (object)$request->dataObj;
        // dd($request['name']);
    ...
    public function addDirectors(Request $request)
    {
        ...
        // $validator = Validator::make($request->all(), [
        //     'exist_id' => 'sometimes',
        //     'd_company_id' => 'required_without:exist_id|numeric',
        //     'd_initial_enquiry_id' => 'required_without:exist_id|numeric',
        //     'd_name' => 'required|string',
        //     'd_email' => 'required|email',
        //     'd_contact_number' => 'required|integer',
        //     'd_date_of_birth' => 'required|date',
        //     'd_additional_shareholdings' => 'required|string',
        //     // 'country_id' => 'required|numeric',
        //     'd_postcode' => 'required|numeric',
        //     'd_full_address' => 'required|string',
        ...
        try {
            if (!empty($request->d_exist_id)) {
                $dataToStore = array(
                    ...
                    'contact_number' => $request->d_contact_number,
                    'date_of_birth' => $request->d_date_of_birth,
                    'additional_shareholdings' => $request->d_additional_shareholdings,
                    'country_id' => $request->d_country_id,
                    'postcode' => $request->d_postcode,
                    'full_address' => $request->d_full_address,
                ...
                $dataToStore = array(
                    'user_id' => auth()->user()->id,
                    'company_id' => $request->d_company_id,
                    'initial_enquiry_id' => $request->d_initial_enquiry_id,
                    'name' => $request->d_name,
                    'email' => $request->d_email,
                    'contact_number' => $request->d_contact_number,
                    'date_of_birth' => $request->d_date_of_birth,
            ...
            if (!empty($request->d_passport)) {
                $file = $request->d_passport;
                $fileName = uniqid() . '.' . $file->getClientOriginalExtension();
                $isFileUploaded = $this->uploadOne($file, config('constants.SITE_MODULE_DOCUMENT_UPLOAD_PATH'), $fileName, 'public');
                if ($isFileUploaded) {
            ...
            if (!empty($request->d_driving_licence)) {
                $file = $request->d_driving_licence;
                $fileName = uniqid() . '.' . $file->getClientOriginalExtension();
                $isFileUploaded = $this->uploadOne($file, config('constants.SITE_MODULE_DOCUMENT_UPLOAD_PATH'), $fileName, 'public');
                if ($isFileUploaded) {
            ...
            if (!empty($request->d_exist_id)) {
                $save = Director::where('id', $request->d_exist_id)->update($dataToStore);
    ...
    {
        // $validator = Validator::make($request->all(), [
        //     'od_company_id' => 'required|numeric',
        //     'od_initial_enquiry_id' => 'required|numeric',
        //     'od_name' => 'required|string',
        //     // 'country_id' => 'required|numeric',
        //     // 'secretary_position' => 'required|numeric',
        // ]);
        // if ($validator->fails()) {
        ...
        DB::beginTransaction();
        try {
            if (!empty($request->od_exist_id)) {
                $dataToStore = array(
                    // 'user_id' => auth()->user()->id,
                    // 'company_id' => $request->company_id,
                    // 'initial_enquiry_id' => $request->initial_enquiry_id,
                    'name' => $request->od_name,
                ...
                $dataToStore = array(
                    'user_id' => auth()->user()->id,
                    'company_id' => $request->od_company_id,
                    // 'country_id' => $request->country_id,
                    'initial_enquiry_id' => $request->od_initial_enquiry_id,
                    'name' => $request->od_name,
                    'secretary_position' => 1,
                    'is_active' => 1,
                );
            ...
            if (!empty($request->od_passport)) {
                $file = $request->od_passport;
                $fileName = uniqid() . '.' . $file->getClientOriginalExtension();
                $isFileUploaded = $this->uploadOne($file, config('constants.SITE_MODULE_DOCUMENT_UPLOAD_PATH'), $fileName, 'public');
                if ($isFileUploaded) {
                    $dataToStore['passport'] = $fileName;
            ...
            if (!empty($request->od_driving_licence)) {
                $file = $request->od_driving_licence;
                $fileName = uniqid() . '.' . $file->getClientOriginalExtension();
                $isFileUploaded = $this->uploadOne($file, config('constants.SITE_MODULE_DOCUMENT_UPLOAD_PATH'), $fileName, 'public');
                if ($isFileUploaded) {
                    $dataToStore['driving_licence'] = $fileName;
            ...
            if (!empty($request->od_exist_id)) {
                $save = Director::where('id', $request->od_exist_id)->update($dataToStore);
            } else {
                $save = Director::create($dataToStore);
            }
    ...
    public function addSupplier(Request $request)
    {
        ...
        try {
            if (empty($request->id)) {
                ...
                $save = Supplier::where('id', (int)$request->id)->update($dataToStore);
            }
            if ($save) {
                DB::commit();
                return $this->responseJson(true, 200, 'Data Added successfully', $save);
    ...
    public function deleteSupplier(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $delete = Supplier::where('id', $request->supplier_id)->delete();
            if (!empty($delete)) {
                DB::commit();
                return $this->responseJson(true, 200, 'Director deleted successfully', []);
            }else{
                return $this->responseJson(false, 500, 'Director deleted Failed', []);
            }
        } catch (\Throwable $e) {
            DB::rollBack();
            logger($e->getMessage() . 'on' . $e->getFile() . 'in' . $e->getLine());
